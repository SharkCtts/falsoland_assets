...
Loop Files, Format("{1}\rule_base\*.*", A_ScriptDir), "R" {
	...
	FileAppend Format("  'visible_{2}':`n    display_name: '&6&m-                                                    -'`n    material: 'ENCHANTED_BOOK'`n    priority: 2`n    slot: {1}`n    lore:`n      - '%shortcut_enchplus/{2}%'`n      - '&f'`n      - '&aPulsa aquí para seleccionar'`n      - '&6&m-                                                    -'`n    click_commands:`n      - '[meta] set %javascript_enchplus_jsview% STRING %shortcut_enchplus/{2}%'`n      - '[meta] set %javascript_enchplus_jsread% STRING {3}'`n      - '[openguimenu] smithingtable_editoverview'", TempSlot, TempFile, TempFull), Format("{1}\output.yml", A_ScriptDir)
	FileAppend Format("`n  'hide_{2}':`n    display_name: '&4&m-                                                    -'`n    material: 'RED_STAINED_GLASS_PANE'`n    priority: 1`n    slot: {1}`n    lore:`n      - '&4&l| &4¡NO DISPONIBLE!'`n      - '&cEsto puede ser a causa de que...'`n      - '&c- Este item no acepta el encantamiento.'`n      - '&c- No has desbloqueado esto en tu arbol'`n      - '&cde progresos.'`n      - '&4&m-                                                    -'`n    view_requirement:`n      requirements:`n        weapon_type:`n          type: `"!string contains`"`n          input: `"%deluxemenus_meta_enchplus-readgroup_STRING%`"`n          output: `"INVALID`"`n`n`n", TempSlot, TempFile, TempFull), Format("{1}\output.yml", A_ScriptDir)
	CategoryLoct += 1
}