...
$execute as @e[sort=random,distance=..5,limit=4,type=!minecraft:player,predicate=ench_plus:status/enchant_target] at @s run function ench_plus:status/chainreaction/macro_03 {"entity_root":"$(entity_root)","entity_tree":"$(entity_tree)","entity_rbgs":"$(entity_rbgs)"}