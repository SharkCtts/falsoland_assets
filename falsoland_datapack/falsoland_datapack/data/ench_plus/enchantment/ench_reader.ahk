...
Loop Files, Format("{1}\rule_base\*.*", A_ScriptDir), "R" {
	...
	FileAppend Format("  'visible_{2}':`n    display_name: '&6&m-                                                    -'`n    material: 'ENCHANTED_BOOK'`n    priority: 2`n    slot: {1}`n    lore:`n      - '%shortcut_enchplus/{3}%'`n      - '&f'`n      - '&aPulsa aquí para seleccionar'`n      - '&6&m-                                                    -'`n    click_commands:`n      - '[meta] set %javascript_enchplus_jsview% STRING %shortcut_enchplus/{3}%'`n      - '[meta] set %javascript_enchplus_jsread% STRING {3}'`n      - '[openguimenu] smithingtable_editoverview'", TempSlot, TempFile, TempFull), Format("{1}\output.yml", A_ScriptDir)
